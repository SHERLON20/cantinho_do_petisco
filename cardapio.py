                                    ...
                                    controls=[
                                       porcao_ou_bebida:= ft.Text(value='bebida'.upper() if e.control.shadow.color == ft.Colors.BLUE_ACCENT else 'caldo'.upper(),
                                            ...
                                            controls=[
                                                ft.Container(col=1),
                                               drop:= ft.Dropdown(options=[ft.dropdown.Option(text=f'{num}')for num in range(1,5)],col=3,icon=None,label='QTD',value=1),
                                               nome_bebida:= ft.Text(value=f'{e.control.content.controls[1].value}'.upper(),text_align=ft.TextAlign.CENTER,size=15,weight=ft.FontWeight.BOLD,col=8),
                                            ],vertical_alignment=ft.CrossAxisAlignment.CENTER
                                        ),
                                        ),
                                       preco:= ft.Text(value=f'preço: {e.control.content.controls[2].value}'.upper(),text_align=ft.TextAlign.CENTER,size=15,weight=ft.FontWeight.BOLD,offset=ft.Offset(x=0,y=1.5))
                                    ],horizontal_alignment=ft.CrossAxisAlignment.CENTER,spacing=5
                                )
                            ),
                            ft.Container(
        ...
        if lista_lanches.content.controls == []:
            ...
            def confirmar(e):
                ...
                if nome.content.controls[0].color == ft.Colors.BLACK and mesa.content.controls[0].color == ft.Colors.BLACK:
                    page.add(tela_confirmar_envio_dados)
